...
args.keep_prob = tf.constant(args.keep_prob, dtype=tf.float64) #CHANGED TO FLOAT64
...
train_queue = DataQueue(data_provider.next_batch, capacity=1000, scope="train_data_queue", dtype=tf.float64)
valid_queue = DataQueue(data_provider.next_valid_batch, capacity=args.num_valid//args.valid_batch_size, scope="valid_data_queue", dtype=tf.float64)
...
with tf.Session() as sess:
    ...
    while not coord.should_stop():
        ...
        step += 1
        _, tmploss, emse, emae, fmse, fmae, qmse, qmae, dmse, dmae = sess.run([train_op, loss_t, emse_t, emae_t, fmse_t, fmae_t, qmse_t, qmae_t, dmse_t, dmae_t], options=run_options, feed_dict={nn.keep_prob: 1.0}, run_metadata=run_metadata)
